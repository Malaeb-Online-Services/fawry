...
class PaymentValidation
{
    ...
    protected static function validateRequiredFields(array $params): void
    {
        ...
        foreach ($requiredFields as $field) {
            if (!isset($params[$field])) {
                throw new PaymentException(Lang::trans('fawry::exceptions.payment.required_field_missing', ['field' => $field]));
            }
        }
    }
    ...
    protected static function validateUserData(array $userData): void
    {
        ...
        foreach ($requiredFields as $field) {
            if (!isset($userData[$field]) || empty($userData[$field])) {
                throw new PaymentException(Lang::trans('fawry::exceptions.payment.required_user_field_missing', ['field' => $field]));
            }
        }
    }
    ...
    {
        if (empty($items)) {
            throw new PaymentException(Lang::trans('fawry::exceptions.payment.items.required'));
        }

        foreach ($items as $index => $item) {
            if (!isset($item['id'], $item['price']) || empty($item['id']) || !is_numeric($item['price'])) {
                throw new PaymentException(Lang::trans('fawry::exceptions.payment.items.invalid'));
            }

            if (isset($item['quantity']) && (!is_numeric($item['quantity']) || $item['quantity'] < 1)) {
                throw new PaymentException(Lang::trans('fawry::exceptions.payment.items.invalid_quantity'));
            }
        }
    }
    ...
    protected static function validateUrls(array $params): void
    {
        if (!filter_var($params['redirect_url'], FILTER_VALIDATE_URL)) {
            throw new PaymentException(Lang::trans('fawry::exceptions.payment.invalid_redirect_url'));
        }

        if (isset($params['webhook_url']) && !filter_var($params['webhook_url'], FILTER_VALIDATE_URL)) {
            throw new PaymentException(Lang::trans('fawry::exceptions.payment.invalid_webhook_url'));
        }
    }
} 